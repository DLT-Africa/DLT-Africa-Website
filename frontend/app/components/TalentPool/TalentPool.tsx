...
const TalentPool: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTalents = async (): Promise<void> => {
      setLoading(true);
      try {
  ...
  const renderButtons = (): JSX.Element[] => {
    ...
    const createButtonClass = (style: ButtonStyle): string => {
  ...
  const renderTalents = (): JSX.Element | JSX.Element[] => {
    if (loading) {
      return (
